...
export async function createDeal(input: DealInput): Promise<{ error?: string }> {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      monthly_bill_satang: bahtToSatang(d.monthlyBillBaht ?? 0),
      estimated_system_size_kwp: d.estimatedSystemSizeKwp ?? null,
      roof_type: nullableText(d.roofType),
      province: nullableText(d.province),
      survey_date: nullableText(d.surveyDate),
      installation_target_date: nullableText(d.installationTargetDate),
      payback_years: d.paybackYears ?? null,
      solar_notes: nullableText(d.solarNotes),
